...
このサービスを提供する上でおすすめの商品の提案方法には2つのアプローチがあって
* ユーザー個人個人に特化したおすすめの商品を提案する方法
* ユーザー全体の購買傾向から流行りのものを提案する方法

どちらのアプローチが最善かを考えた時に、
お客さまがこのサービスを通して幸せを感じれる瞬間は豊かに生活できることだと考えた。
自分の生活にあったものがどんどん増えていけば、よりその人の人生が豊かに明るくなっていく。
そういった考えから、やはり、ユーザー個人個人に特化した商品を学習し提案することが、本サービスの目指すべき目的だと考えました。

おすすめ商品の提案にはAWS Personalizeを採用。
世界一のECサイトであるAmazon.comが利用する機械学習テクノロジーを用いて、機械学習に関する高度な知識がなくても、ユーザー個人個人に特化し高度にカスタマイズされたレコメンデーションをリアルタイムで提供できる。
例えば韓国で60万人以上のユーザーを抱える大手ディスカウントストアのLotte Martでは、ユーザー一人一人にパーソナライズされたおすすめ商品を提案することで、ユーザーの購入頻度を1.7倍に増加させることに成功しました。
よりユーザー個人に特化したおすすめ商品を提案することが、購入頻度の増加に直接的に結びつくことが示されました。
ユーザーへの高いエクスペリエンスの提供と開発コストの削減にはAWS Personalizeの採用は不可欠です。

次に満たすべき機能要件と非機能要件を考えました。
ログインしてから購入するまではECサイトとして機能する上で必要不可欠となる。
また、お勧め商品を提案する機能が今回の重要な機能となり、顧客の興味を学習するために「検索ログを学習する機能」と「購入イベントや買い物カゴ追加イベントから学習する機能」が必要となると考えます。
検索ログからユーザーの興味を学習する理由としましては、例えばユーザーが欲しいものがあるとして、まずはいろんなECサイトで商品情報を検索し、調べることから始まると考えており、最終的にどのサイトで購入するかわからないが、それも紛れもない購買傾向であると言えるからである。

ここで実際に機能のフローを説明する

非機能要件としては、
可用性の観点で、複数のAvailability ZoneにEC2インスタンスやAuroraを配置し、単一障害点となることを回避しました。
また、ECサイトは流行りやトレンドによっても急激にアクセス数が増えることもあり、そういった突発的なアクセス増加に対応するため、Auto Scalingグループを設定してアクセス数によってスケーリングするように設定しました。
Auroraは複数のAvailability Zoneにレプリカを作成でき、即座にフェイルオーバーできる構成となっているため、可用性を引き上げます。
耐障害性では、先ほどの単一障害点の排除に加えて、Cloud WatchによりAWS サービスのログを監視し、障害発生時にSNSから通知して対応できるようにした。
